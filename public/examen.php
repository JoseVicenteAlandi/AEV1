...
El problema es que el ! fa que diga que si dona 0 no siga divisible, la solucio es llevarlo.
*/
...
Conta i torna quants digits te un numero sanser positiu.
$comptador te valor=0 i $i te valor null.
*/
...
Ordena el array $numeros i despres imprimeix el array ja ordenat.
Hi han 72 iteracions.
El valor de $numeros en la itaracio 11 sera = [1, 4, 9, 23, 55, 1, 1, 23, 100].
*/
function hola (){
...
function noFunciona(){
 echo("<BR/><b>Exercici 4 </b><BR/>");
  ...
  $correcte = (isset ($_POST['correcte']))? $_POST['correcte']:"";
  $intents = (isset ($_POST['intents']))? $_POST['intents']:"";
...
function exercici5($lletra){
    ...
    foreach ($mesos as $mes) {
        if (strtolower($mes[0]) === $lletra) {
            $resultat[] = $mes;
        }
    }

    if (empty($resultat)) {
        return "Cap mes comenca per la lletra '{$lletra}'.";
    } else {
        return $resultat;
    }
}
